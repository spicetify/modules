...
The Store provides a recovery link to the installed Spicetify app even when the
background service is unavailable. This path does not require a terminal.

1. Select **Repair Spicetify** in the Store.
2. Read the restart notice, then select **Open Spicetify**.
3. Approve the app prompt. Playback stops during repair.
4. Wait for Spotify to restart, then return to the Store to check the result.

Spotify may silently ignore direct links to other apps. If nothing opens,
copy the **Open Spicetify** link address from its context menu, paste it into
your browser's address bar, and press Enter. Approve **Open Spicetify** in the
browser prompt, then return to the Store after Spotify restarts.
...
If the handler is missing or nothing opens, activating the link does not count
as a successful repair. Repair the Spicetify installation using your
