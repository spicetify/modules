...
For first-party modules, run `pnpm verify <module>` from the repository root.
This builds the module before checking types, lint, formatting, stdlib
boundaries, repository dependency ranges, and its colocated tests. It exits
with a failure as soon as a gate fails. It doesn't replace the full CI checks
when you change shared tooling or stdlib.
...
For lifecycle changes, exercise enable, use, disable, and re-enable. Check that
removed UI stays removed after Spotify changes its DOM, listeners don't run
twice, and neighboring modules retain their state. Shared mounting regression
tests use real React and happy-dom; verify the affected controls in Spotify
before calling a runtime change ready to release.

## Strict type checking

First-party modules can migrate independently to strict TypeScript checking.
Auto Skip Explicit is the first migrated module. To migrate another module:

1. Add `tsconfig.strict.json` in its directory, extending
   `../../tsconfig.module-strict.json`.
2. Run `pnpm verify <module>` and fix the module's strict diagnostics.
3. Commit the configuration with the fixes. `pnpm check` automatically checks
   every module with this configuration, so CI keeps migrated modules strict.

The runner checks every TypeScript source in the selected module, including
files not imported by its entry point. Client sources and Node tests run in
separate type environments. The module's configuration cannot exclude sources
from the verification command.

Each migrated module passes two checks. The baseline check rejects ordinary
type errors in the module and all imported dependencies. The strict check
rejects strict diagnostics in the selected module while reporting a count of
deferred strict diagnostics in legacy module dependencies. Dependency sources
stay in the strict program so their inferred nullable types reach consumers.
This does **not** establish that stdlib is strict.

Use `pnpm verify <module>` or, after building, run
`node scripts/module-typecheck.ts --strict`. Directly running
`tsc -p modules/<module>/tsconfig.strict.json` also reports legacy dependency
strict diagnostics. Standalone projects generated by the kit retain their
existing TypeScript configuration; this incremental migration applies to
first-party modules in this repository.
