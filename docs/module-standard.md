...
Strict TypeScript checking is the default for first-party modules, stdlib,
repository scripts, and TypeScript projects generated by the kit. `pnpm check` checks the whole
repository and every module. New modules participate automatically.

Lyrics Plus retains 12 existing `@ts-nocheck` exemptions in its legacy port.
Those files are not type-checked; their migration remains separate work. Its
tests and typed interfaces participate in strict checks. No other module has a
file-level `@ts-nocheck` exemption.

Run `pnpm verify <module>` to check one module, or, after building, run
`node scripts/module-typecheck.ts --all` to check all modules. Each module check
includes every TypeScript source, even files its entry point doesn't import.
Client sources and Node tests run in separate type environments, so Node globals
don't hide errors in browser code.

Errors in imported dependencies fail verification. Fix the source types and
handle nullable values explicitly; don't defer dependency diagnostics or disable
strict checking for a module. The kit builds its own sources with strict checking
and generates strict `tsconfig.json` files for the basic, extension, and app
templates.
