/*
 * Ambient type surface for the client's `Spicetify` global, so module authors
 * get autocomplete and typechecking on it by default (no import, no cast).
 * Derived from the community Spicetify type definitions; v3-specific members
 * (Modules, _platform) and a TrackMetadata index signature are layered on top.
 * SPDX-License-Identifier: GPL-3.0-or-later
 */
declare namespace Spicetify {
  /** v3 modular loader runtime: Spicetify.Modules.report/list()/manifest/etc. */
  const Modules: any;
  /** Internal platform registry that backs `Platform`. */
  const _platform: any;
  type Icon =
    | "album"
    | "artist"
  ...
  type TrackMetadata = {
    // Track metadata is an open string map; the named keys below are the
    // common ones, but dynamic/indexed access (e.g. `artist_uri:${n}`,
    // "media.type") must stay valid.
