...
import { ProviderLRCLIB } from "./providers/lrclib.ts";
import {
	ProviderMusixmatch,
...
import { ProviderNetease } from "./providers/netease.ts";
...
const Providers = {
	...
	netease: async (info) => {
		...
		let list;
		try {
			list = await ProviderNetease.findLyrics(info, (s) => Utils.toSimplifiedChinese(s));
		} catch {
			result.error = "No lyrics";
			return result;
		}
	...
	lrclib: async (info) => {
		...
		const synced = ProviderLRCLIB.getSynced(list, currentTrackDurationMs());
		if (synced) {
			result.synced = synced;
		}

		const unsynced = synced || ProviderLRCLIB.getUnsynced(list, currentTrackDurationMs());
