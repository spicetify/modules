...
export const OptionsMenuItem = react.memo(({ onSelect, value, isSelected }) => {
	return react.createElement(PopoverMenuItem, { onSelect, selected: isSelected }, value);
});

export const OptionsMenu = react.memo(({ options, onSelect, selected, defaultValue, bold = false }) => {
...
export const TranslationMenu = react.memo(
	({ friendlyLanguage, hasTranslation, musixmatchLanguages, musixmatchSelectedLanguage }) => {
		...
		useEffect(() => {
			// Keep the legacy OptionList listener synchronized while its owned
			// popover remains mounted.
		...
		const updateTranslationSetting = (name, value) => {
			if (name === "translate") {
		...
		return (
			<Popover
				...
				content={
					<div>
						<h3>Conversions</h3>
						<OptionList type="translation-menu" items={items} onChange={updateTranslationSetting} />
					</div>
				}
			>
...
export const AdjustmentsMenu = react.memo(({ mode, hasPerformer }) => {
	return react.createElement(
		Popover,
		{
