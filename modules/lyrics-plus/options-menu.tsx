...
import { OptionList, saveVisualSetting, type SettingItem, type SettingChange } from "./settings.tsx";
import { ProviderMusixmatch } from "./providers/musixmatch.ts";
import * as sharedCallbacks from "./shared-callbacks.ts";

const { useMemo } = react;

export const OptionsMenuItem = react.memo(
	({ onSelect, value, isSelected }: { onSelect: () => void; value: ReactTypes.ReactNode; isSelected: boolean }) => (
		<PopoverMenuItem onSelect={onSelect} selected={isSelected}>
			{value}
		</PopoverMenuItem>
	),
);
...
export const OptionsMenu = react.memo(
	({
		options,
		onSelect,
		selected,
		...
		>
			<button
				...
				aria-label={`Choose an option. Current value: ${selected?.value || defaultValue}`}
			>
...
export const TranslationMenu = react.memo(
	({
		...
		friendlyLanguage?: string | null;
		hasTranslation: { musixmatch: boolean; netease: boolean };
		musixmatchLanguages?: string[] | null;
		musixmatchSelectedLanguage?: string | null;
	}) => {
		...
		const items = useMemo(() => {
			let sourceOptions: Record<string, string> = {
				none: "None",
			};
			...
			let modeOptions: Record<string, string> = {
				none: "None",
			};
			...
			if (availableMusixmatchLanguages.length) {
				...
				const musixmatchOptions = musixmatchOptionsArray.reduce<Record<string, string>>(
					(acc, { key, label }) => {
						acc[key] = label;
						return acc;
					},
		...
		const updateTranslationSetting: SettingChange = (name, value) => {
			if (name === "translate") {
			...
			if (name === "translate:translated-lyrics-source") {
				...
				const nextMusixmatchLanguage =
					typeof value === "string" && value.startsWith(musixmatchTranslationPrefix)
		...
		return (
			<Popover
				...
				content={
					<div>
						<h3>Conversions</h3>
						<OptionList items={items} onChange={updateTranslationSetting} />
					</div>
				}
			>
				<button className="lyrics-config-button" aria-label="Conversions">
...
export const AdjustmentsMenu = react.memo(({ mode, hasPerformer }: { mode: number; hasPerformer: boolean }) => {
	...
	const onChange: SettingChange = (name, value) => {
		saveVisualSetting(name, value);
		if (name === "delay") localStorage.setItem(`lyrics-delay:${client.player.data.item.uri}`, String(value));
		sharedCallbacks.lyricContainerUpdate?.();
	};
	return (
		<Popover
